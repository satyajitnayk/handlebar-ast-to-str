...
export function convertAstToString(
	...
	switch (node.type) {
		...
		case HbsNodeTypes.BlockStatement:
			...
			const blockParams = blockNode.params.map((d) => fn(d, options)).join(' ');
			...
			while (nextInverse) {
				if (nextInverse.body[0]?.type === HbsNodeTypes.BlockStatement) {
					...
					inverseProgram += `{{else ${nextBlockPath} ${nextBlock.params
						.map((d) => fn(d, options))
						.join(' ')}}}${fn(nextBlock.program, options)}`;
			...
			output = `{{#${blockPath}${blockParams ? ' ' + blockParams : ''}${
				blockParamsString ? ' as |' + blockParamsString + '|' : ''
			}}}${blockProgram}${inverseProgram}{{/${blockPath}}}`;
		...
		case HbsNodeTypes.MustacheStatement:
			...
			const mustacheParams = mustacheNode.params
				.map((d) => fn(d, options))
				.join(' ');
		...
		case HbsNodeTypes.SubExpression:
			...
			const subParams = subExpNode.params
				.map((param, index) => {
					if (
						...
						return options.modifiers.map((modify) =>
							modify(fn(param, options)),
						);
					} else {
			...
			output = `(${subExpNode.path.original}${allArgs ? ' ' + allArgs : ''})`;
			break;
